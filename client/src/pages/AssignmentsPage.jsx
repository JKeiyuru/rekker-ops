...
function AssignModal({ open, onClose, onSaved, date }) {
  ...
  useEffect(() => {
    if (open) {
      Promise.all([
        api.get('/users').then((r) => r.data.filter((u) => u.role === 'merchandiser' && u.isActive)),
        api.get('/branches'),
      ]).then(([m, b]) => { setMerchandisers(m); setBranches(b); });
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Assign Merchandisers — {format(new Date(date + 'T00:00:00'), 'dd MMM yyyy')}</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
            {rows.map((row, i) => (
              <div key={i} className="grid grid-cols-[1fr_1fr_100px_auto] gap-2 items-center">
                <Select value={row.merchandiserId} onValueChange={(v) => updateRow(i, 'merchandiserId', v)}>
                  <SelectTrigger className="h-8 text-sm"><SelectValue placeholder="Person…" /></SelectTrigger>
                  <SelectContent>
                    {merchandisers.map((m) => (
                ...
                <Select value={row.branchId} onValueChange={(v) => updateRow(i, 'branchId', v)}>
                  <SelectTrigger className="h-8 text-sm"><SelectValue placeholder="Branch…" /></SelectTrigger>
                  <SelectContent>
                    {branches.map((b) => (
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Assign merchandisers to branches for each day</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-border rounded-xl">
          ...
          <Button className="mt-4" onClick={() => setModalOpen(true)}><Plus className="w-4 h-4" />Add Assignments</Button>
        </div>
      ) : (
        <div className="rounded-xl border border-rekker-border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {assignments.map((a, i) => (
                <tr key={a._id} className={`border-b border-rekker-border/50 hover:bg-accent/20 transition-colors ${i % 2 !== 0 ? 'bg-rekker-surface/20' : ''}`}>
                  ...
                  <td className="px-5 py-3 font-mono text-xs text-foreground">{a.expectedCheckIn || '—'}</td>
                  <td className="px-5 py-3 text-xs text-muted-foreground">{a.assignedBy?.fullName || '—'}</td>
                  <td className="px-5 py-3">
                    <Button size="sm" variant="ghost" className="h-7 px-2 hover:text-destructive hover:bg-destructive/10" onClick={() => handleDelete(a._id)}>
                      <Trash2 className="w-3.5 h-3.5" />
                    </Button>
                  </td>
                </tr>
      ...
      <AssignModal
        ...
        onSaved={(newItems) => setAssignments((prev) => {
