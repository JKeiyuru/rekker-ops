...
// Rows are now clickable — opens the TripSessionDrawer for full detail.

import { useState } from 'react';
import { format } from 'date-fns';
import { Truck, Clock, MapPin, Users, AlertTriangle, ChevronRight } from 'lucide-react';
import { Badge } from '@/components/ui/badge';
import TripSessionDrawer from '@/components/fresh/TripSessionDrawer';
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {sessions.map((s, i) => {
              ...
              const timeAtLoc = s.minutesAtCurrentLocation;
              const isLate    = timeAtLoc != null && timeAtLoc > 90;
              ...
              return (
                <tr
                  ...
                  className={cn(
                    'border-b border-rekker-border/50 hover:bg-accent/20 transition-colors cursor-pointer group',
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-foreground font-medium">{s.currentLocation || '—'}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <span className={cn(
                      'font-mono text-xs font-medium flex items-center gap-1',
                      isLate ? 'text-destructive' : timeAtLoc > 45 ? 'text-amber-400' : 'text-foreground'
                    )}>
                      <Clock className="w-3 h-3" />
                      {minutesToLabel(timeAtLoc)}
                      {isLate && <span className="text-[10px] ml-0.5">LONG</span>}
                    </span>
                  </td>
